...
<body style="font-family: -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; color: #0B1E45; background: #FBEEDC; padding: 24px;">
    <div style="max-width: 560px; margin: 0 auto; background: #ffffff; border: 2px solid #0B1E45; border-radius: 16px; padding: 28px 30px;">
        <h2 style="margin: 0 0 6px; font-size: 20px;">{{ __('New contact request') }}</h2>
        <p style="margin: 0 0 20px; color: #5C5344; font-size: 14px;">{{ __('Submitted through the Festilaw website.') }}</p>

        <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
            <tr>
                <td style="padding: 6px 0; color: #5C5344; width: 130px;">{{ __('Name') }}</td>
                <td style="padding: 6px 0; font-weight: 600;">{{ $submission->first_name }}</td>
            </tr>
            <tr>
                <td style="padding: 6px 0; color: #5C5344;">{{ __('Email') }}</td>
        ...
        <p style="margin: 0; color: #a49a86; font-size: 12px;">{{ __('Reference') }}: {{ $submission->reference }}</p>
    </div>
</body>
</html>
