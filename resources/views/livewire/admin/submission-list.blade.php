<div>
    <div class="mb-6">
        <h1 class="text-xl font-semibold tracking-tight text-slate-900">{{ __('Dossiers') }}</h1>
        <p class="mt-1 text-sm text-slate-500">{{ __('Tous les dossiers clients, du premier contact à la finalisation.') }}</p>
    </div>
    ...
    <div class="mb-4 flex flex-wrap items-end gap-3">
        <div class="min-w-[240px] flex-1">
            ...
            <div class="relative">
                ...
                <input id="f-search" type="search" wire:model.live.debounce.400ms="search" placeholder="{{ __('Email, entreprise, référence...') }}"
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        @if ($submissions->isEmpty())
            <p class="px-6 py-16 text-center text-sm text-slate-500">{{ __('Aucun dossier ne correspond.') }}</p>
        @else
