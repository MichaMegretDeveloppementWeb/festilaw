<div>
    @if ($sent)
        <div class="funnel-success">
            ...
            <h3 class="funnel-success__title">{{ __('Thanks, we\'ve got your details.') }}</h3>
            <p class="funnel-success__text">{{ __('We\'ll reach out shortly to continue your Pro Pack setup.') }}</p>
    ...
    @else
        <form wire:submit="submit" class="funnel-form" novalidate>
            ...
            <div class="hp-field" aria-hidden="true">
                <label for="pf-hp">{{ __('Leave this field empty') }}</label>
            ...
            <div class="funnel-form__row funnel-form__row--two">
                <div class="funnel-form__field">
                    <label for="pf-company">{{ __('Company') }}</label>
                    <input type="text" id="pf-company" wire:model="company_name" placeholder="{{ __('Your company') }}">
                    @error('company_name') <span class="funnel-form__error">{{ $message }}</span> @enderror
                </div>
                <div class="funnel-form__field">
                    <label for="pf-name">{{ __('Your name') }} <span class="funnel-form__optional">{{ __('(optional)') }}</span></label>
                    <input type="text" id="pf-name" wire:model="first_name" autocomplete="given-name" placeholder="{{ __('First name') }}">
                </div>
            </div>

            <div class="funnel-form__field">
                <label for="pf-email">{{ __('Email') }}</label>
            ...
            <div class="funnel-form__field">
                <label for="pf-url">{{ __('Store or website') }} <span class="funnel-form__optional">{{ __('(optional)') }}</span></label>
            ...
            <div class="funnel-form__field">
                <label for="pf-products">{{ __('What do you sell?') }} <span class="funnel-form__optional">{{ __('(optional)') }}</span></label>
                <input type="text" id="pf-products" wire:model="product_types" placeholder="{{ __('e.g. home decor, apparel') }}">
            </div>
